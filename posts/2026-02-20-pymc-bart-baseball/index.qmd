...
author: "Ryan Plain"
date: "2026-02-20"
categories: [Bayesian, PyMC, BART, Python]
...
This was my take at replicating [PyMC Labs BART baseball example](https://www.pymc-labs.com/blog-posts/bayesian-additive-regression-tree-swinging-strikes) and to get experience with BART. A lot of learnings and quirks along the way. One of the biggest gotchas was how BART models are saved to disk and the tree information you lose when starting a new session. I've implemented it below, but it is covered in this [GitHub issue](https://github.com/pymc-devs/pymc-bart/issues/123#issuecomment-3105432020)

Additionally, I built a `bayestidy` package to replicate what [Tidybayes](https://mjskay.github.io/tidybayes/) does in the R ecosystem.

## Imports

To start, I had to leverage Claude to understand a weird quirk to render in quarto and jupyter. The first set of imports handle that, and LLM commented as such.

I have simple `RERUN` flag to compile the models or not. This _can_ be done on a local laptop. I only have 16 gb of memory, so it was a bit of a stretch. To work more efficiently I saved the data to disk and load with context managers to derive dataframes and plots needed from the posterior. This way I don't have all of it in memory at once.
...
Here you can get a sense of how I'm leveraging the `RERUN` flag. All of the I/O has similar conditional logic.
...
The first model replicates PyMC Lab's initial model with 4 features. If my logic is somewhat correct for how they filtered the data, the rest was fairly straightforward.
...
Working with the posterior `az.InferenceData` object turned out to have different behavior than expected.

Essentially I was able to still save the object and load it into new sessions to continue working with the posterior, however the tree information was not saved with the object (that is the extent of my understanding). When I came back to this in a new session, I was able to get the posterior distributions for the parameters, but predictions on the test set were all wrong. I ended up finding a solution documented in this [GitHub issue](https://github.com/pymc-devs/pymc-bart/issues/123#issuecomment-3105432020).

I'm saving the `az.InferenceData` object and additionally a pickle file of the trees. Later on I used the functions in that github issue to do a different style of predictions.
...
Since there are two models, and one small machine, I context manage the inference data and pull out what is needed.
...
Here is where `pymc_bart.utils._sample_posterior()` is needed. I wasn't able to do the traditional PyMC posterior predictions _if_ I started a new session.
...
Really appreciated this metric as I have no domain knowledge over baseball. The pitcher names could be replaced with human-readable uuids and it would have no effect on my ability to work with the data.
...
The next set of work is to implement their improved model, which leveraged a lot of baseball physics that I have little understanding of. I'm just trying to learn BART to use it in a domain I'm familiar with.

What I did find incredibly useful was it's combination with a hierarchical component of park in conjunction with a tree based model. PyMC made this really slick to implement.
...
Similar training splits, I have inconsistent coordinate mappings but it is not worth the couple of hours it takes to run on my machine to work through.
